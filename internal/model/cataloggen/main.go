...
type target struct {
	File, Provider, API string
}

var targets = []target{
	{File: "anthropic.json", Provider: "anthropic", API: "anthropic-messages"},
	{File: "openai-codex.json", Provider: "openai-codex", API: "openai-codex-responses"},
	{File: "openai.json", Provider: "openai", API: "openai-responses"},
}
...
type oracle struct {
	File, Provider, API, SHA256 string
	Count                       int
	IDs                         []string
}
...
func main() {
	...
	for _, target := range targets {
		...
		oracles = append(oracles, oracle{
			File: target.File, Provider: target.Provider, API: target.API,
...
func validateAndCanonicalize(raw []byte, target target) ([]byte, []string, error) {
	...
	if len(groups) != 1 || groups[target.API] == nil {
		return nil, nil, fmt.Errorf("catalog must contain only API %q", target.API)
	}
	models := groups[target.API]
	...
	for id, rawModel := range models {
		...
		if strings.TrimSpace(id) == "" || identity.ID != id || identity.Provider != target.Provider || identity.API != target.API {
...
func render(oracles []oracle, allIDs []string) ([]byte, error) {
	...
	for _, value := range oracles {
		fmt.Fprintf(&out, "\t%q: {Provider: %q, API: %q, SHA256: %q, Count: %d},\n", value.File, value.Provider, value.API, value.SHA256, value.Count)
	}
	out.WriteString("}\n\n")
