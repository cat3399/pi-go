...
func TestDecodeBashInput(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown field", raw: "{\"command\":\"x\",\"cwd\":\"/\"}", wantFailure: true},
		{name: "duplicate field", raw: "{\"command\":\"x\",\"command\":\"y\"}", wantFailure: true},
		...
		{name: "lone surrogate", raw: "{\"command\":\"\\ud800\"}", wantFailure: true},
