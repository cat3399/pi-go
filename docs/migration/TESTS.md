...
| `T-SESSION-008` | B-SESSION-008 | `packages/coding-agent/test/session-manager/migration.test.ts` 的 v1->v2/idempotent cases | `implemented-awaiting-review` | Open consumer v1 chain/compaction index、v2 hook rename/id preservation、atomic publication/adoption fault/idempotence；Go strengthening covers strict malformed/partial, backup-only explicit recovery, limits, production no-overwrite，及 Windows-only high-range identity-lock read/append + alias rejection、open-destination `FileRenameInfoEx`/migration/recovery runtime suite；非 Windows seam 仅验证 ABI contract |
