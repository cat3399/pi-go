...
| `T-SESSION-012` | B-SESSION-013/014/016 | `compaction.test.ts`, `compaction-serialization.test.ts`, `session-manager/build-context.test.ts` 的 manual prepare、latest summary、retained tail、wire intent | `implemented; review pending` | v3 checkpoint/reopen/fork、selected sibling isolation、prompt snapshot、usage/cost and malformed first-kept matrix |
...
| `T-SESSION-014` | B-SESSION-015 | coding/Harness `compaction.ts::{estimateContextTokens,findCutPoint,shouldCompact}` | `implemented; review pending` | assistant-usage/trailing estimate, tool-result-safe cut, MaxUint64 usage+tail/cut-prefix overflow、no-write regression and fuzz; automatic trigger remains deferred |
