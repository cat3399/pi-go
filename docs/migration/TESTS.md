...
| `T-SESSION-009` | B-SESSION-005/006 | `packages/coding-agent/docs/session-format.md` tree contract 与 `SessionManager._buildIndex/buildSessionPath` physical-tail behavior | `strengthened` | branch-tail 接受；multiple-root/forward/cycle 拒绝；Open→Append 原 byte prefix 不变 |
...
| `T-AUTH-002` | B-AUTH-002 | `auth-storage.ts` mode 0600 intent；上游无 atomic durability/Windows ACL evidence | `strengthened` | permission、pre-rename fault/temp cleanup、Windows-only fail-closed suite/cross-compile |
| `T-AUTH-003` | B-AUTH-003 | `auth-storage.test.ts` concurrent modifications | `strengthened` | same/different Store、local/file wait cancellation、two contending process writers、failure release/merge、`-race` |
