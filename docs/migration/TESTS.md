...
v0.2-rich-content-replay 均已通过独立复审；provider/tools 联合集成仍单独验收。
...
| `T-PROVIDER-011` | B-PROVIDER-006 | `packages/ai/test/openai-responses-partial-json-cleanup.test.ts` — function-call argument cleanup cases | `strengthened` | final JSON object validation, delta-prefix reconciliation and no executable partial call；provider/tools `fa78b62` 联合集成待验收 |
| `T-PROVIDER-012` | B-PROVIDER-006 | `openai-responses-shared.ts::convertResponsesMessages/processResponsesStream`；`openai-responses-{partial-json-cleanup,foreign-toolcall-id,message-id,empty-tool-result,terminal-event}.test.ts` | `strengthened` | reasoning/text/image rich replay、provenance/restart、Azure terminal backfill 与 phase/order failure由 R-BASE-003 通过；完整 provider/tools workflow 等 `fa78b62` 联合集成 |
