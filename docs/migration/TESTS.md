...
| `T-SESSION-007` | B-SESSION-004/005 | 上游无 byte-prefix unknown round-trip、partial write、atomic create/fsync、poison/commit-unknown test | `strengthened` | fault/cancel/poison、byte-prefix matrix；Anthropic/Google、future/malformed signature raw fork/reopen 与 safe projection 由 R-BASE-003 通过 |
| `T-SESSION-008` | B-SESSION-005/008 | `packages/coding-agent/test/session-manager/migration.test.ts` 的 v1->v2/idempotent cases | `strengthened` | Open consumer v1 chain/compaction index、v2 hook rename/id preservation、atomic publication/adoption fault/idempotence；联合 oracle 覆盖 legacy rich/future/unknown raw 保留与安全投影、identity writer exclusion、v3 rich reopen/append 不 rewrite、recovery selected branch，以及 production/core retry context；Windows-only high-range identity-lock read/append + alias rejection、open-destination `FileRenameInfoEx`/migration/recovery suite 已交叉编译但未在真实 Windows runtime 执行；R-SESSION-005，且不扩大 R-BASE-003/R-PROVIDER-005/R-AGENT-003 的历史范围 |
...
| `T-SESSION-015` | B-SESSION-018 | `branch-summarization.ts`, `branch-summary-extensions.test.ts`, tree navigation paths | `deferred` | requires M-SESSION/v0.5 navigation owner + cache/invalidation + cancel/publication contract; no partial API |
...
| `T-AUTH-006` | B-AUTH-006 | `openai-codex-oauth.test.ts` browser URL/callback/manual intent | `strengthened` | PKCE/state, bind, request limits, cancellation, flushed page, Wait/Close/late callback and explicit opener；R-AUTH-002 |
...
| ID | Behavior | 上游 test intent | 当前状态 | 目标与重评条件 |
| --- | --- | --- | --- | --- |
| `T-TUI-001` | B-TUI-001 | `packages/tui/test/stdin-buffer.test.ts` — split CSI/mouse, Kitty, paste, timeout/flush and destroy cases | `strengthened` | large chunk streaming, WezTerm double-ESC, exact pending/paste bounds, maximal-run invalid UTF-8, split/EOF and fuzz regression；R-TUI-001 |
| `T-TUI-002` | B-TUI-002 | `packages/tui/test/keys.test.ts` legacy/CSI-u/modifyOtherKeys/keypad/event cases | `strengthened` | strict malformed grammar, canonical ID, layout-base, lock/keypad/event matrix；remaining upstream functional families deferred |
| `T-TUI-003` | B-TUI-003 | `wrap-ansi`, `truncate-to-width`, `tab-width`, regional-indicator regression tests | `strengthened` | upstream Myanmar/Indic/RI fixtures plus indentation/empty-line wrap, exact padded wide/empty-ellipsis truncate, ZWJ/control boundary and stateful SGR/OSC-8 slice regressions；R-TUI-001 |
| `T-TUI-004` | B-TUI-004 | `terminal.test.ts`, `terminal-colors.test.ts` negotiation/dimensions/color intent | `strengthened` | partial raw restore, late negotiation, concurrent balance and fake lifecycle；真实 PTY/Windows runtime smoke deferred to interactive assembly |
| `T-TUI-005` | B-TUI-005 | `word-navigation.test.ts` whitespace/punctuation/CJK cases | `strengthened` | Go UTF-8-byte navigation tests；R-TUI-001 |
