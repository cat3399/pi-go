...
| `D-AUTH-001` | `auth.json` provider credential map | `packages/coding-agent/src/core/auth-storage.ts`；`packages/ai/src/auth/types.ts` | mode 0600、lock/merge、malformed 不覆盖、secret 不进入 log | `classified` |
...
M-APP/v0.2 只增加 OpenAI `api_key` credential 的只读 projection，用于 production
assembly；它不写文件、不实现 login/logout，也不把 D-AUTH-001 标记为完整迁移。未知 provider
entry 原样不触碰，malformed 或不支持的已选 credential 明确失败且不得回退环境变量。
