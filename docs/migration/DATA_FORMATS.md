...
| `D-SETTINGS-001` | global/project `settings.json` | `packages/coding-agent/src/core/settings-manager.ts` | source precedence、project trust、unknown setting preservation | `ported-v0.1`; project read gated by explicit D-TRUST-001 decision |
| `D-TRUST-001` | `~/.pi/agent/trust.json` project trust decisions | `packages/coding-agent/src/core/trust-manager.ts::ProjectTrustStore` | lexical admission、nearest ancestor、lock/merge、raw unknown preservation；Windows private persistence fail-closed | `ported-strengthened-v0.1-unix` |
...
| `D-RESOURCE-001` | global/project `prompts/` 与 legacy `commands/` | `packages/coding-agent/src/migrations.ts::migrateCommandsToPrompts` | prompt discovery/expansion `ported-strengthened`；legacy commands migration deferred，绝不丢 source | `partially-ported-strengthened` |
| `D-RESOURCE-002` | global/project extensions、skills、themes 与 package resource dirs | `packages/coding-agent/src/core/resource-loader.ts`；`packages/coding-agent/src/migrations.ts` | global/trusted-project skills `ported-strengthened`；extensions/themes/package dirs deferred | `partially-ported-strengthened` |
