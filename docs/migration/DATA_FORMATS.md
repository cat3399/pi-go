...
| `D-MODEL-002` | dynamic `models-store.json` | `packages/coding-agent/src/core/models-store.ts`；`packages/ai/src/models-store.ts` | provider-scoped merge、etag/checkedAt、malformed 不覆盖；runtime read-only projection、opaque entry/model fields write-back preservation | `classified`; active M-MODEL review, remote refresh deferred |
