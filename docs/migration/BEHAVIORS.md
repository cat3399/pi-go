...
| `B-SESSION-012` | selected path extract 与 full forest fork 创建新 header/parentSession，source 不覆写，target publication/cancel 可诊断；commit-unknown source fail-closed | `session-manager.ts::createBranchedSession/forkFrom`；tree/custom-id tests；Go atomic strengthening | `in-progress` | active aggregate snapshot、poison quarantine、external strict open 与 fault/race 回归已提交；M-SESSION/v0.2 独立复审通过后再定稿状态 |
