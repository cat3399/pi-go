...
| `B-PROVIDER-006` | OpenAI Responses function schema、assistant/function-call + ToolResult replay、reasoning/encrypted replay、source-order SSE reducer、tool-result images 与 partial/unknown failure | WF-003 | `in-progress` | M-AGENT/v0.1 single-call 接入；多 call dispatch 等 M-AGENT/v0.2；custom/cache/image executor deferred |
