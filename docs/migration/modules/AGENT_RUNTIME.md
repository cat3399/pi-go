...
- threshold 自动 compaction 每个 logical provider turn 最多一次；明确 context-overflow 另有且仅有一次
  compact-and-retry admission。每个 retry 都重新取得 immutable context snapshot。conflict/cancel/summary error/commit-unknown 均
...
- 最终联合 oracle 在 trusted system prompt 下组合 rich reasoning/text、parallel multi-tool、source-order
  durable replay 与第二 turn transient stream retry；两个 retry request 的 causal transcript 相同，
  session tool results 不重复，Summarizer request 不广告 Agent tools。
...
- 不实现 TUI、tool wire replay、mixed length/tool terminal 或 Harness 独立 runtime；这些保持原有
  deferred behavior。`R-AGENT-003` 仍只覆盖 `5d0099d`、`eff0ad7`、`a419519`；随后完成的
  `c8d1d1c` + rich + context 最终 integration 不追溯扩张该 review 范围。
