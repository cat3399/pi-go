...
阶段编号表达依赖关系，不要求每个阶段只能发布一次。每个阶段都可以拆成多个小的、
可验收的 feature slice。
...
退出条件：可以从清单中选择首批 feature slice，并能说明它们的依赖、行为证据
和测试来源。
...
- 迁移 message/content 类型、纯数据转换、stream 辅助逻辑和稳定错误分类。
...
退出条件：基础 package 可以独立测试，fixture 可重复，默认测试不需要 Node.js、
...
- 实现一组满足真实 workflow 的内置 tool。
- 通过 print/headless mode 提供第一个可使用的 pi-go executable。

该阶段不建设 plugin system、remote API 或 pi-web adapter。

退出条件：一个包含 model 调用、tool 执行和 session 持久化的 E2E scenario 完全
运行在 Go 中，进程树中不存在 TypeScript pi。
...
2. 生成源码、测试、数据格式和依赖差异。
3. 编码前先分类新增、改变和删除的行为。
4. 更新 behavior ledger、test ledger 和受影响的 fixture。
5. 实现差异，或明确记录暂缓与有意不同的理由。
6. 清单内部一致后，再更新 docs/UPSTREAM.md。
